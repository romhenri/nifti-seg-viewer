#!/usr/bin/env python3
"""Generate index.html for the NIfTI Segmentation Viewer.

The HTML is a single self-contained page that loads niivue from a CDN and
renders NIfTI volumes with an optional segmentation overlay entirely in the
browser. Run this script to (re)produce index.html:
...
<body>
  <div id="controls">
    <label>Base: <input type="file" id="baseFile" accept=".nii,.nii.gz" /></label>
    <label>Overlay: <input type="file" id="overlayFile" accept=".nii,.nii.gz" /></label>
    <label>Slice type:
      <select id="sliceType">
